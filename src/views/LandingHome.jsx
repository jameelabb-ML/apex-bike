...
import { suitTypes, teamPresets, perforationOptions, protectionOptions, heroImages, reviews, faqs } from '../data/configOptions.js';
...
const LandingHome = ({ onNavigate, onAddToCart }) => {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* TRUST STRIP — lives only on the landing page, scrolls away naturally */}
      <section className="bg-orange-500/[0.06] border-y border-orange-500/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-1.5 text-[11px] sm:text-xs font-medium text-orange-300/90 text-center">
            <span>Free Worldwide Shipping on Orders $2,500+</span>
            <span className="hidden sm:inline text-orange-500/30">•</span>
            <span>5-Year Structural Warranty</span>
            <span className="hidden sm:inline text-orange-500/30">•</span>
            <span>CE Category II Certified</span>
          </div>
        </div>
      </section>
            ...
            {teamPresets.map((preset) => (
              <div key={preset.id} className="group bg-slate-900/40 rounded-2xl border border-slate-800 overflow-hidden hover:border-orange-500/20 transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_20px_40px_-10px_rgba(0,0,0,0.5),0_0_20px_rgba(249,115,22,0.05)]">
                ...
                <div className="p-5">
                  <div className="flex items-center gap-1 mb-2">
                    {[...Array(5)].map((_, i) => (
                      <svg key={i} className={`w-3.5 h-3.5 ${i < Math.floor(preset.rating) ? 'text-orange-400' : 'text-slate-700'}`} fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                    ))}
                    <span className="text-[11px] text-slate-500 ml-1">({preset.reviews})</span>
                  </div>
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    <span className="text-[10px] px-2 py-0.5 rounded bg-slate-800 text-slate-400 border border-slate-700">
                      {suitTypes.find(s => s.id === preset.suitType)?.name.replace(/ Full Track Suit| Street-Tuned Suit/, '')}
                    </span>
                    <span className="text-[10px] px-2 py-0.5 rounded bg-slate-800 text-slate-400 border border-slate-700">
                      {protectionOptions.find(p => p.id === preset.protection)?.name}
                    </span>
                  </div>
                  ...
                  <button onClick={() => onNavigate('presets', preset.id)} className="w-full bg-slate-800 hover:bg-orange-500 text-slate-300 hover:text-white font-medium py-2.5 rounded-lg border border-slate-700 hover:border-orange-500 transition-all duration-300">
                    Select Preset
                  </button>
                </div>
              </div>
